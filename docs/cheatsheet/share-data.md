...
- **Server-Side**: when generating a token
    ...
    - [API REST](/reference-docs/REST-API#post-apitokens): add body parameter `"data"` to the POST operation
    - [openvidu-java-client](/reference-docs/openvidu-java-client#generate-a-token): construct TokenOptions with `TokenOptions.Builder().data("USER_DATA").build()`
    - [openvidu-node-client](/reference-docs/openvidu-node-client#generate-a-token): construct TokenOptions with `{data: "USER_DATA"}` <br><br>
