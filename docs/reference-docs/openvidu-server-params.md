...
| `openvidu.publicurl`               | URL to connect clients to OpenVidu Server. This must be the full IP of your OpenVidu Server, including _protocol_, _host_ and _port_ (for example: `https://my.openvidu.server.ip:4443`). If no _port_ argument is provided, `server.port` param will be appended to it | ***local***<br>(with default value _local_ this parameter will be set to `localhost:PORT`, being _PORT_ the param `server.port`) |
