...
| **`OPENVIDU_PRO_NETWORK_QUALITY`**     | Whether to enable or disable network quality. If true, the webrtc stats will be checked and examined with the aim of know the network quiality | **false** |
| **`OPENVIDU_PRO_NETWORK_QUALITY_INTERVAL`**     | If `OPENVIDU_PRO_NETWORK_QUALITY=true`, how often the network quality algotithm will be invoked, in seconds | **5** |
