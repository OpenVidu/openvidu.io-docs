...
| **`OPENVIDU_FORCED_CODEC`** | Possible values are **VP8** and **H264**. This parameter will ensure that all the browsers use the same codec, avoiding transcoding process in the media server, which result in a reduce of CPU usage. Recommended and default value is **VP8** because it is compatible with all browsers. | **VP8** |
| **`OPENVIDU_ALLOW_TRANSCODING`** | Defines if transcoding is allowed or not when `OPENVIDU_FORCED_CODEC` is not a compatible codec with the browser. If this parameter is false, not compatible browsers with codec specified in `OPENVIDU_FORCED_CODEC` will fail. | **false** |
...
| **`OPENVIDU_PRO_ELASTICSEARCH_MAX_DAYS_DELETE`** | Maximum number of days that Elasticsearch indexes in rollover state will be kept on disk | **15** |
