...
| **`OPENVIDU_WEBHOOK`** | Whether to enable the [OpenVidu Webhook](reference-docs/openvidu-server-webhook/){:target="_blank"} service or not. | **false** |
...
| **`OPENVIDU_STREAMS_FORCED_VIDEO_CODEC`** | This parameter will ensure that all the browsers use the same codec, avoiding transcoding process in the media server, which result in a reduce of CPU usage. Possible values are:<ul><li>`VP8` (Recommended and default value because its compatibility with browsers)</li><li>`H264`</li><li>`NONE` (No codec will be forced)</li></ul> | **VP8** |
...
| **`OPENVIDU_EDITION`** | OpenVidu Edition to run. Possible values are:<ul><li>`pro`: Run an **[OpenVidu Pro](openvidu-pro/){:target="_blank"}** cluster.</li><li>`enterprise`: Run an **[OpenVidu Enterprise](openvidu-enterprise/){:target="_blank"}** cluster.</li></ul> | **pro** |
...
| **`OPENVIDU_PRO_NETWORK_QUALITY`** | Whether to enable or disable the **[Network Quality API](advanced-features/network-quality/){:target="blank"}**.<br>You can monitor and warn users about the quality of their networks with this feature. | **false** |
...
| **`OPENVIDU_PRO_RECORDING_STORAGE`** | Where to store recordings:<ul><li>`local`: Use local storage.</li><li>`s3`: Use an AWS S3 bucket</li></ul>See **[Uploading recordings to AWS S3](advanced-features/recording/#uploading-recordings-to-aws-s3){:target="blank"}**. | **local** |
