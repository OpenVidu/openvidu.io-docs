...
| **`OPENVIDU_PRO_DOCKER_REGISTRIES`** | List of Docker registries to be used by Media Nodes. The list is a JSON array of strings, each one with the following properties:<br>`"serveraddress=test.io,username=test,password=test"` | **[ ]** |
...
| **`MEDIASOUP_IMAGE`** | Docker image used in Media Nodes for Mediasoup in OpenVidu Pro/Enterprise ([see](https://docs.openvidu.io/en/stable/releases/)). If you have created your own mediasoup image and it is stored in a private Docker registry, you can configure it using property `OPENVIDU_PRO_DOCKER_REGISTRIES` | |
