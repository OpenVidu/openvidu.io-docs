...
| **`MEDIASOUP_IMAGE`** | Docker image used in Media Nodes for mediasoup in OpenVidu Pro/Enterprise. By default it is the correspondant image of the deployed version ([see](https://docs.openvidu.io/en/stable/releases/)). If you have created your own mediasoup image and it is stored in a private Docker registry, you can configure it using property `OPENVIDU_PRO_DOCKER_REGISTRIES` | |
