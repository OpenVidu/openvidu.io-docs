...
> - **forceVideoCodec** _(optional String)_ : This parameter will ensure that all the browsers use the same codec, avoiding transcoding process, which result in a reduce of CPU usage. Recommended and default value is `VP8` because it is compatible with all browsers.
...
>     - `H264`
> - **allowTranscoding** _(optional Boolean)_ : Defines if transcoding is allowed or not when `forceVideoCodec` is not a compatible codec with the browser. If this parameter is `false`, not compatible browsers with codec specified in `forceVideoCodec` will fail. Default value is `false`. You can change this default value by setting a different value to `OPENVIDU_ALLOW_TRANSCODING` in [OpenVidu Configuration](reference-docs/openvidu-config)
##### Returns
