...
>  **WARNING**: This media node will not have any configuration in `/opt/kms/.env`. All configuration parameters of Kurento Media Server are defined in OpenVidu Server Pro Node using the configuration file in `/opt/openvidu/.env`. You can check more info about how to configure Media Nodes [here](deployment/pro-enterprise/on-premises/#32-configuration)
