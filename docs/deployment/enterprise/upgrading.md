...
- **[Migrating from 2.32.1 to 2.32.2](#migrating-from-2321-to-2322)**
...
## Migrating from 2.32.1 to 2.32.2
...
- **[Migrating from 2.32.1 to 2.32.2 (AWS Cloudformation - Single master deployment)](#migrating-from-2321-to-2322-aws-cloudformation-single-master-deployment)**
- **[Migrating from 2.32.1 to 2.32.2 (On premises - Single master deployment)](#migrating-from-2321-to-2322-on-premises-single-master-deployment)**
- **[Migrating from 2.32.1 to 2.32.2 (AWS Cloudformation - High Availability deployment)](#migrating-from-2321-to-2322-aws-cloudformation-high-availability-deployment)**
- **[Migrating from 2.32.1 to 2.32.2 (On Premises - High Availability deployment)](#migration-from-2321-to-2322-on-premises-high-availability-deployment)**

### Migrating from 2.32.1 to 2.32.2 (AWS Cloudformation - Single master deployment)
<br>

Upgrading process for this kind of deployment is exactly the same as [Migrating from 2.32.1 to 2.32.2 OpenVidu Pro (AWS with Cloudformation)](https://docs.openvidu.io/en/2.32.2/deployment/pro/upgrading/#migrating-from-2321-to-2322-aws-cloudformation).

> Remember to use `OPENVIDU_EDITION=enterprise` to run the deployment as OpenVidu enterprise.

### Migrating from 2.32.1 to 2.32.2 (On premises - Single master deployment)
<br>

Upgrading process for this kind of deployment is exactly the same as [Migrating from 2.32.1 to 2.32.2 OpenVidu Pro (On premises)](https://docs.openvidu.io/en/2.32.2/deployment/pro/upgrading/#migrating-from-2321-to-2322-on-premises).

> Remember to use `OPENVIDU_EDITION=enterprise` to run the deployment as OpenVidu enterprise.

### Migrating from 2.32.1 to 2.32.2 (AWS Cloudformation - High Availability deployment)
<br>
...
aws s3api create-bucket \
    --bucket <new-bucket-v2.32.2> \
    --region <your-region>
aws s3 sync s3://<bucket-v2.32.1> s3://<new-bucket-v2.32.2>
```
...
**3)** Deploy the Cloudformation template of 2.32.2 from [this instructions](https://docs.openvidu.io/en/2.32.2/deployment/enterprise/aws/#high-availability-deployment). If you've followed steps 1 and 2, put at the **`OpenVidu S3 bucket`** parameter your new bucket, otherwise just keep the parameter blank.

**4)** Wait for the deployment to be in [healthy state](https://docs.openvidu.io/en/2.32.2/deployment/enterprise/aws/#check-cluster-after-deploy)

**5)** Check your new S3 bucket (or check the `new-bucket-v2.32.2` you have created if you followed steps 1 and 2), and move your old configuration to the `.env` configuration of the new bucket. You can follow [this section](https://docs.openvidu.io/en/2.32.2/deployment/enterprise/aws/#2-change-configuration-by-modifying-s3-configuration-file-not-recommended) to see how to change and restart OpenVidu master nodes.

<br>

### Migration from 2.32.1 to 2.32.2 (On Premises - High Availability deployment)
...
<code id="code-1"><strong>curl https://s3-eu-west-1.amazonaws.com/aws.openvidu.io/install_ov_enterprise_ha_base_2.32.2.sh | bash -s upgrade</strong></code>
...
```console
================================================
Openvidu Enterprise HA base services successfully upgraded to version v2.32.2
================================================

1. A new file 'docker-compose.yml' has been created with the new OpenVidu Enterprise HA v2.32.2 services

2. The previous file '.env' remains intact, but a new file '.env-v2.32.2' has been created.
Transfer any configuration you wish to keep in the upgraded version from '.env' to '.env-v2.32.2'.
When you are OK with it, rename and leave as the only '.env' file of the folder the new '.env-v2.32.2'.

3. If you were using Openvidu Call application, it has been automatically updated in file 'docker-compose.override.yml'.
However, if you were using your own application, a file called 'docker-compose.override.yml-v2.32.2'
...
If you want to rollback, all the files from the previous installation have been copied to folder '.old-2.32.1'
```
...
<p style="text-align: start">
<code id="code-2"><strong>curl https://s3-eu-west-1.amazonaws.com/aws.openvidu.io/install_openvidu_enterprise_ha_node_2.32.2.sh | bash -s upgrade
</strong></code>
...
```console
================================================
Openvidu Enterprise HA successfully upgraded to version v2.32.2
================================================
