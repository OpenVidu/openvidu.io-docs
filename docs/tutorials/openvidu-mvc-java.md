...
A secure OpenVidu sample app with a Java backend and a traditional MVC frontend. It makes use of _openvidu-java-client_ to connect to OpenVidu Server. With regard to the use of OpenVidu, it is identical to [openvidu-js-java](/docs/tutorials/openvidu-js-java){:target="_blank"}. This tutorial is intended for developers who feel more comfortable with MVC web architectures for their frontends. [Thymeleaf](http://www.thymeleaf.org/){:target="_blank"} is the template engine of choice for this tutorial.
